#!/usr/bin/env python3
"""Train M1: M0 genus proxy objective + explicit cross-locus view invariance.

Scientific firewall
-------------------
* Gradients: TRAIN_REF only.
* Proxy supervision: ITS-core only (same as M0).
* New M1 term: core <-> randomly sampled ITS1/ITS2 view contrastive alignment.
...
from itsnet.openworld.losses import view_invariance_loss
...
def main():
    ...
    print(f"objective       : proxy(core) + {args.lambda_view:g} * L_view(core<->ITS1/ITS2)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_loss = total_proxy = total_view = total_pair_cos = 0.0
        total_n = correct = 0
        for cores, alts, ids, y, alt_views in loader:
            y = y.to(device, non_blocking=True)
            # One forward pass; core and alternate views share a padding length.
            ...
            with torch.amp.autocast(device_type=device.type, dtype=torch.float16, enabled=amp_enabled):
                ...
                logits = head(z_core)  # proxy supervision remains core-only, as in M0
                proxy_loss = F.cross_entropy(logits, y)
                view_loss = view_invariance_loss(
                    z,
                    ids + ids,
                    temperature=args.view_temperature,
                )
                loss = proxy_loss + args.lambda_view * view_loss
        ...
        print(
            ...
            f"pair_cos={rec['train_pair_cosine']:.3f}  proxy_acc={rec['train_proxy_accuracy']:.3f}  "
            f"lr={rec['lr']:.2e}"
        )

    ckpt = {
        "experiment": "M1_core_proxy_plus_view_invariance",
    ...
    torch.save(ckpt, outdir / "m1_checkpoint.pt")

    # Evaluate all 3x3 query/reference locus combinations on DEV only.
    ...
    with open(outdir / "m1_metrics.json", "w") as fh:
    ...
    with open(outdir / "m1_dev_predictions.tsv", "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=pred_fields, delimiter="\t")
        w.writeheader(); w.writerows(all_preds)

    print(f"\nwrote {outdir / 'm1_checkpoint.pt'}")
    print(f"wrote {outdir / 'm1_metrics.json'}")
    print(f"wrote {outdir / 'm1_dev_predictions.tsv'}")
