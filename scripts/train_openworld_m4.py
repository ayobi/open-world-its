#!/usr/bin/env python3
"""Train M4: M1 view invariance + taxonomy geometry + pseudo-novel episodes.

Scientific firewall
-------------------
* Gradients: TRAIN_REF only.
* Proxy supervision: ITS-core only (identical to M0/M1/M3).
* View term: core <-> randomly sampled ITS1/ITS2 (identical to M1/M3).
* Taxonomy term: hierarchical neighbourhood geometry on core embeddings (M2).
* Episode term: each eligible core query is treated as pseudo-novel at genus rank;
  it must retrieve its family from learned genus proxies after its own genus proxy
  has been explicitly removed from that family's prototype (M3).
...
def make_sampler(ds: PairedFullDataset):
...
def main():
    ...
    ap.add_argument("--skip-dev-eval", action="store_true",
                    help="train and save checkpoint without embedding/scoring DEV; used for locked post-development retraining")
    args = ap.parse_args()
    ...
    ds = PairedFullDataset(rows, genus_to_idx)
    ...
    print(
        ...
        f"{args.lambda_view:g} * L_view + "
        f"{args.lambda_taxonomy:g} * L_taxonomy(core) + "
        f"{args.lambda_episode:g} * L_episode(leave-one-genus family)"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_loss = total_proxy = total_view = total_tax = total_episode = total_pair_cos = 0.0
        ...
        for cores, alts, ids, y, alt_views, lineages in loader:
            ...
            with torch.amp.autocast(device_type=device.type, dtype=torch.float16, enabled=amp_enabled):
                ...
                loss = (
                    ...
                    + args.lambda_episode * episode_loss
                )
        ...
        print(
            ...
            f"tax={rec['train_taxonomy_loss']:.4f}  episode={rec['train_episode_loss']:.4f}  "
            f"pair_cos={rec['train_pair_cosine']:.3f}  "
            f"proxy_acc={rec['train_proxy_accuracy']:.3f}  lr={rec['lr']:.2e}"
        )

    ckpt = {
        "experiment": "M4_M1_plus_taxonomic_geometry_plus_leave_one_genus_family_episode",
    ...
    torch.save(ckpt, outdir / "m4_checkpoint.pt")
    ...
    if args.skip_dev_eval:
        report = {
            ...
            "scientific_firewall": {
                ...
                "development_evaluation": "SKIPPED",
                "reason": "locked post-development retraining; no DEV/CAL/TEST scoring",
            },
        ...
        with open(outdir / "m4_metrics.json", "w") as fh:
            json.dump(report, fh, indent=2, sort_keys=True)
        print(f"\nwrote {outdir / 'm4_checkpoint.pt'}")
        print(f"wrote {outdir / 'm4_metrics.json'}")
        print("DEV/CAL/TEST evaluation skipped by request")
        return
    ...
    with open(outdir / "m4_dev_predictions.tsv", "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=pred_fields, delimiter="\t")
        w.writeheader(); w.writerows(all_preds)

    print(f"\nwrote {outdir / 'm4_checkpoint.pt'}")
    print(f"wrote {outdir / 'm4_metrics.json'}")
    print(f"wrote {outdir / 'm4_dev_predictions.tsv'}")
